...
class RocketMqTest {
    ...
    public void send() throws InterruptedException, MQBrokerException, RemotingException, MQClientException {
         ...
         *
         * 提交如果返回失败 则进入发送重试，根据相应的策略（messageDelayLevel16个等级）进行重试(ACK)
        ...
        /**
         * rocketMq 分布式事务
         * 背景：场景1：在一个事务，一条sql操作 一个发送，如果先发送mq 在操作数据库，发送结束mq 也被消费了，
         * 结果后半部分代码报错 事务回滚，但是数据已经被消费了
         * 场景2 ：在一个事务，一条sql操作 一个发送，先执行sql，然后发送mq sql执行成功，
         *  mq因为网络原因，发送消息异常 ，消费者永远得不到消息
         *
         *  基于场景1，2
         *
         * 解决方案：正常事务
         * 使用rocketmq的分布式事务 即先提交一个半消息发送给broker broker标记不能被消费者看到，
         * 然后提交本地事务 半消息根据本地事务的提交状态判断这条消息是commint还是rollback
         *
         * 问题：如果本地事务处理结束生产者发送commint还是rollback 时超时或者其他异常
         *
         * 解决方案 ：事务信息的补偿流程
         * 在本地创建一条事务表（存事务id）本地事务commint,则插入事务id到数据库
         * 如果broker 长时间没有收到本地事务的执行状态，则会想生产者发送确认回查的请求
         * 生产者收到请求 则检查本地事务是否成功（即事务表是否有数据）
         * broker根据状态来判断commit 还是rollback
         */
/*        //同步发送 对结果很关注
        for (int i=0;i<=100;i++){
